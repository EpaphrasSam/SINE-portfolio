    ...
    return (
      ...
      >
        <div className="flex items-start gap-4">
          <div
            className={`flex-shrink-0 w-12 h-12 rounded-lg flex items-center justify-center transition-colors duration-150 relative overflow-hidden ${
              isSelected
                ? "bg-white/20"
...
const ProjectSection = memo(
  ({
    ...
    const currentIndex =
      selectedProject && isTypeSelected
...
export default function Projects() {
  ...
  // Handle URL changes (both from history and direct)
  useEffect(() => {
    ...
    // If no hash is present and no project selected, select the first web project
    if (!window.location.hash && !selectedProject) {
      const firstWebProject = projects.find((p) => p.type === "web");
      if (firstWebProject) {
        setSelectedProject(firstWebProject.id);
        window.history.replaceState(null, "", `#${firstWebProject.id}`);
      }
    }
